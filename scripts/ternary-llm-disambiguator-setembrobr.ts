...
import { computeTernaryMetrics, predictTernaryLockedEnsembleRows, readTernaryOofScores, readTernaryTestScores } from "../src/ternary.ts";
import type { TernaryEnsembleLock, TernaryLockedPredictionRow } from "../src/types.ts";

type Mode = "oof" | "test";

const mode = arg("--mode") as Mode | undefined;
if (mode !== "oof" && mode !== "test") throw new Error("Usage: bun run scripts/ternary-llm-disambiguator-setembrobr.ts --mode oof|test");
...
if (!config.outputDir.includes("symmetric")) throw new Error("LLM disambiguator is scoped to the symmetric raw lane");
...
if (mode === "oof") {
  ...
  await writeJson(resolveTernaryOutputPath(config, "reports", "llm-disambiguator-oof-report.json"), {
    dataset: "setembrobr",
    seed: config.seed,
    lane: "raw_qwen3_symmetric",
    lockBasename,
  ...
  await writeJson(resolveTernaryOutputPath(config, "reports", "llm-disambiguator-test-manifest.json"), {
    dataset: "setembrobr",
    seed: config.seed,
    lane: "raw_qwen3_symmetric",
    lockBasename,
...
async function readLockedScores(modeValue: Mode, currentLock: TernaryEnsembleLock) {
  const out = new Map();
  for (const modelId of currentLock.modelIds) {
    const path = resolveTernaryOutputPath(
      config,
      "scores",
      `${modeValue === "oof" ? "train_oof" : "test_score"}_${currentLock.labelPolicyId}_${modelId}.csv`,
    );
    const rows = modeValue === "oof" ? await readTernaryOofScores(path) : await readTernaryTestScores(path);
...
async function decide(row: TernaryLockedPredictionRow, pack: LlmTimelinePack): Promise<LlmDecision> {
...
function decisionFromPayload(
  row: TernaryLockedPredictionRow,
  requestHash: string,
...
async function sourceHashes(modeValue: Mode, currentLock: TernaryEnsembleLock): Promise<Record<string, string>> {
